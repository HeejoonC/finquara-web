  ...
  return (
    <main>
      ...
      <section className="min-h-[calc(100vh-64px)] flex flex-col items-center justify-center bg-white text-center px-6">
        ...
        <p className="text-xl text-gray-500 mb-3">The Global Finance & Risk Talent Network</p>
        <p className="text-sm text-gray-400 mb-10">금융·계리 분야 전문 인재와 기업을 연결합니다</p>
        ...
        <div className="grid grid-cols-3 gap-6 max-w-2xl w-full text-left">
          {[
            { icon: '🎯', title: '계리 전문', desc: '계리사, 보험수리 전문 채용공고 특화' },
            { icon: '🌏', title: '글로벌 네트워크', desc: '국내외 금융·보험 업계 연결' },
            { icon: '✅', title: '검증된 공고', desc: '관리자 검토를 거친 신뢰할 수 있는 공고' },
          ].map(f => (
      ...
      <section className="py-20 px-6 bg-[#0B1F3A] text-center">
        <h2 className="text-2xl font-bold text-white mb-2">지금 바로 등록하세요</h2>
        <p className="text-white/60 text-sm mb-8">새로운 채용공고와 업계 소식을 가장 먼저 받아보세요.</p>
        ...
        <p className="text-white/30 text-xs mt-6">Starting with Actuarial Careers in Korea</p>
      </section>
    </main>
  )
}
